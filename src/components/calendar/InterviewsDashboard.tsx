...
function EventCard({
  ...
  return (
    <div
      className={`p-4 border rounded-lg bg-card card-lift hover:border-foreground/20 ${isPastEvent ? "opacity-60 grayscale-[0.5]" : ""}`}
    >
                ...
                {upcomingEvents.length === 0 ? (
                  ...
                  <div className="space-y-3 stagger-list">
                    {upcomingEvents.map((e) => (
                      <EventCard key={e.id} event={e} onSelect={select} />
                    ))}
                  </div>
                )}
              </div>
